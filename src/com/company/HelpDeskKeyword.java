...
public class HelpDeskKeyword {
    ...
    public static String filename = "HelpDesk 분석_2019_2022_0310.xlsx";	//파일명 설정

    public static List<List<String>> data = readExcel(path,filename);
    ...
    public static void main(String[] args) {
        ...
        //Map<String,Double> detailTwoSimilarityMap = detailSimilarity(column7List,detailOneSimilarityMap.entrySet().stream().collect(Collectors.toList()));

        detailOneSimilarityMap.entrySet().stream()
    ...
    public static Set<String> removeZosa(Set<String> distinctWordSet){
        ...
        String twoZosa ="이나/이고/이라/이랑/에서/가면/에서/에게/한테/으로/않음/안됨/되지/";
    ...
    public static Map<String,Double> detailSimilarity(List<String> columnList, List<Map.Entry<String,Double>> similarityWordList){
        Map<String, Double> detailMap = new HashMap<>();

        //상위 20개의 키워드만 가지고 연산
        for (int i = 0; i < 20; i++) {
            Set<String> set = detailDistinctWordSet(columnList,similarityWordList.get(i).getKey());



            List<Map.Entry<String,Double>> list = checkDetailSimilarity(columnList,set,similarityWordList.get(i).getValue());
            ...
            for (int j = 0; j < list.size(); j++) {
                if (detailMap.get(list.get(j).getKey()) != null){
                    ...
                    double value = Math.round(list.get(j).getValue() * similarityWordList.get(i).getValue() * 100)/100.0;
    ...
    private static List<Map.Entry<String, Double>> checkDetailSimilarity(List<String> columnList, Set<String> detailDistinctWordSet, Double value) {
        ...
        for (String str : detailDistinctWordSet) {
            ...
            for (int i = 0; i < columnList.size(); i++) {
                if (columnList.get(i).contains(str)){
                    ...
                    double result = Math.round(count / (value * data.size())* 1000)/1000.0;
